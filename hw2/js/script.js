   ...
   render() {
      return `<div class="product__card shadow filter-card trans">
               ...
               <button class="card-hover button-hover4 trans1">
                  <svg class="card-hover__img trans" width="26" height="24" viewBox="0 0 32 29" fill="none" xmlns="http://www.w3.org/2000/svg">
                     <path class="trans"
...
class ProductsList {
   ...
   render() {
      ...
      this.products.forEach(product => {
         const goodItem = new ProductsItem(product.id, product.title, product.descProduct, product.price);
         listHtml += goodItem.render();
      });
      document.querySelector('.product__grid-wrap--catalog').innerHTML = listHtml;
   }
   calcSum() {
      let sum = 0;
      this.products.forEach(product => {
