...
//--------------------------------------------
let cartButtonOpen = document.querySelector('.header-right__cart');
...
//-------------catalog--------------------------
class ProductsItem {
   constructor(id, title, descProduct, price) {
      this.id = id;
      this.title = title;
   ...
   render() {
      return `<div id="0${this.id}" class="product__card shadow filter-card trans">
               <img src="img/product-card${this.id}.jpg" width="360" height="420" alt="Product's card" class="product-card__img">
               <a href="product.html" class="product-card__name">
                  <h5 class="product-card__h5">${this.title}</h5>
                  <p class="product-card__text">${this.descProduct}</p>
                  <p class="product-card__text-options">$${this.price}</p>
               </a>
               <button data-id=${this.id} class="card-hover button-hover4 trans1">
                  <svg data-id=${this.id} class="card-hover__img trans" width="26" height="24" viewBox="0 0 32 29" fill="none" xmlns="http://www.w3.org/2000/svg">
                     <path data-id=${this.id} class="trans"
                  ...
                  <span data-id=${this.id} class="card-hover__text trans">Add to Cart</span>
               </button>
            </div>`;
   }
}
class ProductsList {
   ...
   fetchProducts() {
      makeGETRequest('json/catalogData.json')
         .then(data => {
            this.products = data;
            this.render();
         })
   ...
   render() {
      this.products.forEach(product => {
         const goodItem = new ProductsItem(product.id_product, product.title, product.descProduct, product.price);
         document.querySelector('.product__grid-wrap--catalog').insertAdjacentHTML("beforeend", goodItem.render());
      });
   }
}
...
class CartProductsItem {
   ...
   render() {
      return `<div id="1${this.id}" class="shopping__card shadow">
                  <a href="product.html" class="shopping-card__link">
                     <img class="shopping-card__img scale trans" src="img/product-card${this.id}.jpg" width="262"
                  ...
                  <button type="submit" class="shopping-card__close">
                     <i class="fas fa-times removeCard" data-id=${this.id}></i>
                  </button>
               </div>`;
   }
}
...
class CartProductsList {
   ...
   addProduct() {
      let addProd = document.getElementsByClassName("card-hover");
      for (let add of addProd) {
         add.onclick = ev => {
            this.render(ev.target.dataset.id);
            this.removeProduct();
         }
      }
   }
   removeProduct() {
      let removeProd = document.getElementsByClassName("removeCard");
      for (let rem of removeProd) {
         rem.onclick = e => {
            let cards = document.getElementById("1" + e.target.dataset.id);
            cards.remove();
         }
      }
   }
